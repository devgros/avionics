	...
	protected function prePersistEntity($entity)
	{
		...
		if(sizeof($last_dossier)>0){
			...
			if($current_year > $last_year){
				...
				$new_num = str_pad(($last_num+1), 3, "0", STR_PAD_LEFT)."-".$current_year."-AVIONICS";
			}
		}else{
			$new_num = "026-".$current_year."-AVIONICS";
		}
		...
		if ($editForm->isSubmitted() && $editForm->isValid()) {
			...
			if(!$devis){
				...
				if(sizeof($last_devis)>0){
					...
					$current_year = date("y");
					if($current_year > $last_year){
						$new_num = "D001-".$current_year."-AVIONICS";
					}else{
						$new_num = 'D'.str_pad(($last_num+1), 3, "0", STR_PAD_LEFT)."-".$current_year."-AVIONICS";
					}
				}else{
					$new_num = "D001-".date("y")."-AVIONICS";
				}
